...
def berserk(content: str, key: int) -> str:
    ...
    content = _lines_sep_.join(
        str(ord(x) + _hey_num_) if x != "ζ" else _spaces_sep_ for x in _content_
    )
    ...
    _1_ = (
        rf"""_n5_""",
        rf"""lambda _n9_:"".join(chr(int(_n10_)-len(_n9_.split('{_lines_sep_}')))if _n10_!='{_spaces_sep_}'else'ζ'for _n10_ in str(_n9_).split('{_lines_sep_}'))""",
    )
    _2_ = (
        rf"""_n6_""",
    ...
    _6_ = rf"""_n8_""", rf"""lambda _n12_:_n1_(_n12_)"""
    ...
    _final_content_ = (
        rf"""# Obfuscated by : Ferly Afriliyan
...
Kontolivo(_n1_=False,_sparkle='''{content}''')""".strip()
...
printer(
    Panel(
        text,
        title=f"{J2}[ {P2}Banner {J2}]",
        subtitle=f"{P2}┌─ {J2}[ {P2}Input {J2}]",
        subtitle_align="left",
        width=70,
        padding=1,
        style="#FF8F00",
