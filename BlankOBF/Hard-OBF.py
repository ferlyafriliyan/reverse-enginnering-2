...
class BlankOBF:
    ...
    def encrypt1(self):
        ...
        self.code = f"""
{init};
__import__({self.encryptstring("builtins")}).exec(
    __import__({self.encryptstring("marshal")}).loads(
        __import__({self.encryptstring("base64")}).b64decode(
            __import__({self.encryptstring("codecs")}).decode(
                {var1}, 
                __import__({self.encryptstring("base64")}).b64decode("{base64.b64encode(b'rot13').decode()}"
                ).decode()
            ) + {var2} + {var3}[::-1] + {var4}
        )
    )
...
""".strip().encode()
    ...
    def encrypt2(self):
        ...
        conf = {"getattr": var4, "eval": var3, "__import__": var8, "bytes": var9}
        encryptstring = self.encryptor(conf)

        self.code = f"""
{var3} = eval({self.encryptstring("eval")});
{var4} = {var3}({self.encryptstring("getattr")});
{var8} = {var3}({self.encryptstring("__import__")});
{var9} = {var3}({self.encryptstring("bytes")});
{var5} = lambda {var7}: {var3}({encryptstring("compile")})(
    {var7}, 
...
try:
    {encryptstring('__import__("builtins").exec', func=True)}(
        {var5}(
            {encryptstring('__import__("lzma").decompress', func=True)}(
                {var9}({var2})
            )
        )
    ) or {encryptstring('__import__("os")._exit', func=True)}(0)
...
if __name__ == "__main__":
    try:
        clear()
        sourcefile = input("Input file to Obfuscate: ")

        if not os.path.isfile(sourcefile):
            printerr(f'No such file: "{sourcefile}"')
            sys.exit(1)
        elif not sourcefile.endswith((".py", ".pyw")):
            printerr("The file does not have a valid python script extension!")
            sys.exit(1)

        outputfile = input('output file path [Default: "Obfuscated_<FILE>.py"]: ')
        if not outputfile:
            outputfile = "Obfuscated_" + os.path.basename(sourcefile)

        with open(sourcefile) as sourcefile:
            code = sourcefile.read()

        BlankOBF(code, outputfile)
