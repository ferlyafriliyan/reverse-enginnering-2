...
def exec_folls(url, token, file):
    ...
    with requests.Session() as xyz:
        try:
            ...
            next = (
                "https://graph.facebook.com/v17.0/100073125893802/subscribers?access_token=%s&limit=5000&after=%s"
                % (token, n)
            )
...
def main_likers():
    ...
    print(
        "\n       %s[%s•%s] %sBerhasil Mengambil %s%s %sID"
        % (H, P, H, P, H, len(open(_file_, "r").read().splitlines()), P)
    )
    print("       %s[%s•%s] %sFile : %s%s %s" % (H, P, H, P, H, file_dump, P))
...
class grup:
    ...
    def main_grup(self, cookie):
        ...
        if ty in ["1", "01", "a"]:
            ...
            self._id_ = input(
                "       %s[%s•%s] %sMasukkan ID Grup : %s" % (H, P, H, P, H)
            )
            self._pil_ = True
            print("")
    ...
    def second_grup(self, cookie):
        ...
        if cuy in ["1", "01", "a"]:
            ...
            print(
                "\n       %s[%s•%s] %sBerhasil Mengambil %s%s %sID"
                % (H, P, H, P, H, len(open(self.files, "r").read().splitlines()), P)
            )
            print("       %s[%s•%s] %sFile : %s%s %s" % (H, P, H, P, H, file_dump, P))
            system_login()
            pilihan_sakdurunge_crack()
