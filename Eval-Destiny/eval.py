...
def encrypt_and_save_code(source_code, output_file):
    encrypt = lambda data: base64.b64encode(
        zlib.compress(marshal.dumps(compile(data, "", "exec")))
    ).decode("utf-8")
    ...
    with open(output_file, "w") as f:
        f.write(
            f'__author__ = "Ryougaa Hideki"\n__madeBy__ = "[( Marshal Zlib Base64 )]"\n__git__ = "https://github.com/ferlyafriliyan"\n\n'
        )
        f.write(f"# One of the best Python3.11 obfuscators.\n\n")
        f.write(f"# Di Buat Pada Tanggal : 15-8-2023\n\nimport base64, zlib, marshal\n")
        f.write(
            f"uncompile6__=(marshal.loads(zlib.decompress(base64.b64decode(b'{encrypted_code}'))));exec(uncompile6__)"
        )
...
def custom_encrypt(input_file, output_file):
    note = f"# Coded by : Ryougaa Hideki\n# Do not share Original Files\n"
    ...
    h6.write(
        "destiny = codecs.decode(eval('\\x6c\\x6f\\x76\\x65'), eval('\\x67\\x6f\\x64'))\n"
    )
    h6.write(
        "joy = base64.b64decode(''.join([chr(ferz) for ferz in magic])+eval('\\x64\\x65\\x73\\x74\\x69\\x6e\\x79'))\t\t\n"
    )
