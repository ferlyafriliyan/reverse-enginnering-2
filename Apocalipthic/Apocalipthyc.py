...
class Apocalipthyc:
    def __init__(
        ...
        if addbuiltins:
            p("Adding the __builtins__...")
            self.AddBuiltins()

        p("Creating default __vars__...")
        self.CreateVars()
...
{0}()['{3}']={2}('builtins').vars"""[
    ...
    def RenameImports(self):
        ...
        if _imports == False:
            print(
                f"Star import detected! Skipping the renaming of imported libraries and variables."
            )
    ...
    def Compress(self):
        ...
        __lambda___ = f"""(lambda {arg1}:{eval_var}({compile_var}({str_var}("{self._hex(eval_var)}({arg1})"),filename='{self._hex(self._randvar())}',mode='{self._hex('eval')}')))"""
        __lambda____ = f"(lambda {arg1}:{arg1}(__import__('{self._hex('zlib')}')))"
        __lambda_____ = f"(lambda {arg1}:{arg1}['{self._hex('decompress')}'])"

        __lambda__ = [__lambda___, __lambda____, __lambda_____]

        __lambda______ = f"""(lambda {arg2},{arg1}:{arg2}({arg1}))"""
        __lambda_______ = (
            f"""(lambda:{__lambda___}('{self._hex("__import__('builtins').exec")}'))"""
        )

        __lambda__2 = [__lambda______, __lambda_______]

        shuffle(__lambda__)
        shuffle(__lambda__2)

        keys = {lamb: self._randvar() for lamb in __lambda__}

        keys2 = {lamb: self._randvar() for lamb in __lambda__2}
        ...
        decompress = f"{keys[__lambda_____]}({keys[__lambda____]}({keys[__lambda___]}('{self._hex('vars')}')))"
        exec_content = f"{keys2[__lambda_______]}()({keys2[__lambda______]}({decompress},{compressed}))"
...
""".strip()
    ...
    def _randvar2(self):
        return "".join(
            choice("ferlyafriliyan, Apocalipthyc") for _ in range(randint(5, 20))
        )
    ...
    def _add___lambda__(self):
        for _ in range(10):
            lamb = self._randvar()
            arg = self._randvar()
            self.strings[lamb] = f"lambda {arg}:{self._randglob()}()"
            self.__lambda__.append(lamb)

    def _fake_lin(self, indent):
        return f"{' ' * indent}if {choice(list(self.ostrings.keys()))}:\n{' ' * indent * 2 if indent else ' '}{choice(self.__lambda__)}({choice(list(self.ostrings.keys()))})"
