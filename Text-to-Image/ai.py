...
class Main:  # line:15
    ...
    def GeneratorImage(self, prompt, headers):  # line:17
        while True:  # line:18
            ...
            new_headers.update(
                headers.headers
            )  # Update with headers from the response object
            try:
                request_image = self.session.post(
                    self.url_generator_image, headers=new_headers, json=params
                )  # line:20
    ...
    def Home(self):  # line:24
        ...
        panel_info = Panel(
            """[#00BFFF]   Open link : https://pastebin.com/2D0t2xSj
   Disclaimer : ChatGPT
   Decrypt by : Ferly Afriliyan [#808080] V : [1.1.0]
        """,
            title="[#FF8F00][ [#00FF33]By Ferly Afriliyan [#FF8F00]]",
            subtitle="[#FF8F00][ [#00FF33]Create images using text[#FF8F00]]",
            style="#FFFFFF",
        )
        ...
        print(
            f"{K}[{H}#{K}] {P}Client Host : {H}{self.read_root.json()[client_host_key]}"
        )
        ...
        console.print(
            Panel(
                "[green]" + self.prompt,
                title=f"[green]( [white]Prompt [green])",
                style=white_color,
                width=40,
            )
        )
        ...
        for link in self.result_url:  # line:36
            ...
            open(result_image_path + link.replace(self.url_image, ""), "wb").write(
                response.content
            )  # line:39
        ...
        console.print(
            Panel(
                f"[green]Success Created {self.NumberOfImages} Image Saved In:{result_image_path}",
